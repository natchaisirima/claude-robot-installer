...
def main():
    ...
    s,w=d["session_pct"],d["weekly_pct"]; ss,ws=d.get("session_sev"),d.get("weekly_sev")
    rows=d.get("models") or ([{"name":"Opus","pct":d["opus_pct"],"sev":None}] if d.get("opus_pct") is not None else [])
    worst=max([s,w]+[int(m.get("pct",0)) for m in rows])
    sf,sr,sc=fmt(d.get("session_reset_iso","")); wf,wr,_=fmt(d.get("weekly_reset_iso",""))
    title=f"🤖 {s}% ⏰ {sc} 📅 {w}%"+"".join(f" 🧠 {int(m.get('pct',0))}%" for m in rows)
    ...
    for m in rows:
        ...
        if m.get("reset_iso"):
            mf,mr,_=fmt(m["reset_iso"]); print(f"  resets in {mr}  ·  {mf} | color=#888888 size=12")
    print("---")
...
function row(st,em,lb,p,sub){const c=colorFor(p);const rw=st.addStack();rw.layoutVertically();const tp=rw.addStack();tp.centerAlignContent();const e=tp.addText(em+" ");e.font=Font.systemFont(11);const l=tp.addText(lb);l.font=Font.mediumSystemFont(12);l.textColor=Color.white();if(sub){const s=tp.addText("  "+sub);s.font=Font.systemFont(9);s.textColor=DIM;s.lineLimit=1;}tp.addSpacer();const pc=tp.addText(p+"%");pc.font=Font.boldSystemFont(13);pc.textColor=c;rw.addSpacer(3);const im=rw.addImage(barImage(p,c));im.imageSize=new Size(260,6);}
async function build(){const data=await loadData();const w=new ListWidget();const g=new LinearGradient();g.colors=[BG_TOP,BG_BOT];g.locations=[0,1];w.backgroundGradient=g;w.setPadding(11,15,11,15);
